...
void compute_mutual_knn(int n, int k,
    ...
    vector<int> m_hat(n, -1);
    vector<int> cluster_membership(n, -1);
    int n_chosen_points = 0;
    int n_chosen_clusters = 0;
    //半径从小到大，密度从大到小

    std::sort(knn_radii.begin(), knn_radii.end(), std::greater<std::pair<double, int>>());
    ...
    //半径换密度，不等式变向,两个while改了三处符号,常数也得变化
    for (int i = 0; i < n; ++i) {
        ...
        while(knn_radii[i].first < knn_radii[last_considered].first * (1. - beta))  {
    ...
    for (int s = 0; s < n_chosen_points; ++s) {
         result[m_hat[s]] = cluster_membership[s];
    }
