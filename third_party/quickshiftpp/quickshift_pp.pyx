...
class QuickshiftPP:
    ...
    def __init__(self, k, beta,sigma,
                    epsilon=0,
                    ann="kdtree"):
        self.k = k
        self.beta = beta
    ...
    def fit(self, X):
        ...
        if self.ann == "kdtree":
            ...
            knn_radius = query_res[0]
            neighbors = query_res[1]

        memberships = np.zeros(n, dtype=np.int32)
        result = np.zeros(n, dtype=np.int32)
        neighbors = np.ndarray.astype(neighbors, dtype=np.int32)
        knn_radius_temp = np.ndarray.astype(knn_radius, dtype=np.float64)

        for i in range(X.shape[0]):
            kernel_values = 1 / np.sqrt(2 * np.pi) * np.exp(-0.5 * (knn_radius_temp[i]/self.sigma)**2)
            knn_radius[i] = np.sum(kernel_values) / (self.k * self.sigma)
        knn_radius=np.ndarray.astype(knn_radius, dtype=np.float64)  #实际是密度，不是半径
